...
Finally, it is worth mentioning that INEGI differentiates the register date from the ocurrence date. Usually the number of homicides that are reported refer to the register date, but, since INEGI has a comprehensive archive that includes medical records, they also have information regarding the date when the homicide ocurred and not simply when it was registered. Our interest is focused on the date of ocurrence, which is the measurement that we include in the report.[@dataINEGI] 
